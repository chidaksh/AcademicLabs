...
bool compare(P const &p1, P const &p2)
{
    ...
    {
        return ((p1.BurstTime[p1.cpu_burst_index] < p2.BurstTime[p2.cpu_burst_index]) && p1.occupied == false && p1.pseudoArrivalTime != 0 && p1.exited == false && p1.ionotdone == false);
    }
}
...
int SJFscheduler(P *process, int *burstTimeArr, int num_proc)
{
    int time = 0;
    ...
    for (int i = 0; i < num_proc; i++)
    {
        process[i].WaitingTime = -process[i].ArrivalTime;
    ...
    while (!ShortestJobQueue.empty())
    {
        ...
        if (process[currentjob].cpu_burst_index == 0 && process[currentjob].LastRunTime == 0 && process[currentjob].exited == false)
        {
            ...
            // cout << "Response Time of " << process[currentjob].pid << " is " << process[currentjob].ResponseTime << endl;
        }
        time += process[currentjob].BurstTime[process[currentjob].cpu_burst_index]/2;
        for (int i = 0; i < num_proc; i++)
        {
        ...
        if (process[currentjob].cpu_burst_index == burstTimeArr[process[currentjob].pid] - 1)
        {
        ...
        for (int j = 0; j < num_proc; j++)
        {
            if (j != p1.pid)
            {
                if (process[j].exited == false)
                {
                    // cout << " Process with " << process[j].pid << " has Last Run time " << process[j].LastRunTime << endl;
                    process[j].WaitingTime += process[currentjob].BurstTime[process[currentjob].cpu_burst_index];
                    // cout << " Waiting time for Proess " << process[j].pid << " is " << process[j].WaitingTime << endl;
                }
                if (process[j].LastRunTime != 0 && process[j].cpu_burst_index != 0 && process[j].exited == false && process[j].cpu_burst_index > process[j].io_burst_index)
                {
                    ...
                    if (iooccupied == false)
                    {
                        if (process[j].LastRunTime == ioexecution)
                        {
                            ...
                            else
                            {
                                ...
                                process[j].leftiotime = temp - (time - ioexecution);
                            }
                        }
                    }
                    else if (iooccupied = true)
                    {
                        ...
                        else
                        {
                            ...
                            process[iorunning].leftiotime = temp - (time - ioexecution);
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < num_proc; i++)
        {
            ...
            {
                if (process[i].exited == false and time >= process[i].ArrivalTime)
                {
                    leftoverjobs += 1;
                    ShortestJobQueue.push_back(process[i]);
                }
        ...
        {
            time += process[currentjob].BurstTime[process[currentjob].io_burst_index]/2;
...
int main(int argc, char **argv)
{
    ...
    // }
    int TotalTime = 0;
    TotalTime = SJFscheduler(proc, burstTimeArr, num_proc);
