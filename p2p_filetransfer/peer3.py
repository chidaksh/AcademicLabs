    ...
    def fetch_file_fragments(self):
        ...
        while True:
            ...
            if len(self.requested_files) == 0:
                ...
                return
            for file_name in self.requested_files:
                # print("starting semp")
                ...
                if self.available:
                    ...
                    self.currfile = file_name
                    perf = threading.Thread(target=self.ForEachFile,args=(file_name,))
                    perf.daemon = True
                    perf.start()
                    perf.join()
                    self.fileFlag = self.listening_port
                    ...
                    if self.numpeers != 0:
                        ...
                        self.fraglen = math.ceil(self.file_size//self.numpeers)
                    else:
                        self.stop = True
                    for (peer, port) in list(self.downfrompeers.keys()):
                        ...
                        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                            try:
                                ...
                                if self.lastFrag:
                                    req = json.dumps(
                                    {"task": "fetch_file", "filename": file_name, "start": self.last, "last": self.file_size})
                                    s.send(req.encode())
                                else:
                                    req = json.dumps(
                                        {"task": "fetch_file", "filename": file_name, "start": self.last, "last": self.last+self.fraglen})
                                    s.send(req.encode())
                                # print("request sent")
                                # s.send(f"fetch_file:{file_name}".encode())
                                try:
...
if __name__ == "__main__":
    peer = Peer("Nikhil", peer_ip="127.0.0.5", peer_listeningport=6500)
    print(peer.name)
    try:
        peer.add_peer()
        peer.read_peers()
        peer.get_shareable_files(["file2.txt"])
        peer.get_ToBeRequested_files(['file1.txt'])
